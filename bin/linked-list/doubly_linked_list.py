...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 29 18:44:51 2020
...
class DoubleLinkedList(object):
    ...
    def insert_after(self, data, x):
        ...
        if p is None:
            ...
            if p.next is not None:
                p.next.prev=temp    #should not to be done when p refers to last node
            p.next=temp
    ...
    def delete_node(self, x):
        if self.start is None:         #list is empty
            return
        if self.start.next is None:         #list hs only one node
            if self.start.info==x:
                self.start=None
            else:
                print(x, ' not found')
        ...
        if p.next is not None:      #nod to be deleted is in between
            p.prev.next=p.next
            p.next.prev=p.prev
