...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  6 12:25:33 2020

@author: deepak
"""

class Node(object):         #implemented a single linked list as circular list
...
class CircularLinkedList(object):
	...
	def __init__(self):
		self.last=None 		#In constructor we initilize it to None
	...
	def display_list(self):
		...
		p=self.last.link 	#1st node of the list
		while True:
			print(p.info, ' ', end='')
			p=p.link
			if p==self.last.link: 	#we will stop ehen p-> 1st node in circular list
				break
		print()
	...
	def delete_last_node(self):
		...
		p=self.last.link	#to find second last node
	...
	def delete_node(self, x):
		...
		if self.last.link.info==x:	#deletion of 1st node
			self.last.link=self.last.link.link
			return
			
		p=self.last.link	#to find predessor of the node
		...
		if p.link==self.last.link:
			...
			p.link=p.link.link
			if self.last.info==x:
				self.last=p	#this ref.'last' has to  be updated!
